...
def publish(episode_path: Path, mp3_path: Path, public_dir: Path) -> None:
    ...
    metadata = {
        ...
        "product_name": episode["product"]["name"],
        "product_url": episode["product"]["url"],
    ...
    product_url = html.escape(episode["product"]["url"], quote=True)
    page = f"""<!doctype html>
...
<p><a href="{product_url}">See the full product details on OneFantasticShop.net</a></p>
